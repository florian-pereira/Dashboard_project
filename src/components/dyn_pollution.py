...
path_pollution = os.path.join(
    current_dir, "..", "..", "data", "cleaned", "annual-co-emissions-from-aviation.csv"
)
path_airports = os.path.join(
    current_dir, "..", "..", "data", "cleaned", "airports_cleaned.csv"
)
...
df_clean = pd.merge(
    df_clean, df_routes, left_on="Entity", right_on="Country", how="left"
)
...
full_index = pd.MultiIndex.from_product(
    [all_countries, all_years], names=["Entity", "Year"]
)
...
fig.update_xaxes(
    showgrid=True, gridcolor="#333", title=None
)  # Supprime les titres X individuels
